...
def create_post(session: Session, title, text, tag_id, user_id):
    new_post = Post(title=title, text=text, tag_id=tag_id, user_id=user_id)
        ...
        for post in range(posts_quantity):
            try:
                create_post(
                    ...
                    random.randint(1, session.query(Tag).count()),
                    random.randint(1, session.query(User).count())
                )
            except ValueError:
                create_tag(session, fake.user_name())
                create_user(session, fake.user_name(), fake.ascii_email())
    if tags_quantity:
